            ...
            if (event.getAction() === MotionEvent.ACTION_UP) {
                if (event.rawX >= binding.SignInPassword.right - binding.SignInPassword.compoundDrawables[Right].bounds.width()) {
                    ...
                    passwordVisible = if (passwordVisible) {
                        ...
                        //for hide password
                        binding.SignInPassword.transformationMethod = PasswordTransformationMethod.getInstance()
                        false
                    } else {
                        //set drawable image here
                        binding.SignInPassword.setCompoundDrawablesRelativeWithIntrinsicBounds(0, 0, R.drawable.visibility, 0)
                        //for show password
                        binding.SignInPassword.transformationMethod =
                            HideReturnsTransformationMethod.getInstance()
                        true
                    }
                    binding.SignInPassword.isLongClickable = false //Handles Multiple option popups
